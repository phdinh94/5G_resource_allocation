...
data_dir    =  '/home/dinhp/data/wowmom_ext/ca/verizon/second_bs/' 

#verizon single
operator    =   'verizon_second_bs'
...
def preprocess_df(csv_file):
    ...
    #verizon is seperated by a bursts of very low throughput, the following
...
def bar_plot_average_smart_tput_single_user(run_nums):
    """
    Plot average throughput per sending rate for all runs, single user
    """
    csv_file_list   =   []
    ...
    for csv_file in csv_file_list:
        ...
        sub_df_list =   get_per_rate_sub_df(preprocess_df(csv_file))
        for sub_df in sub_df_list:
            avgs += round(np.mean(sub_df[smart_tput_col])).tolist()
            stds += round(np.std(sub_df[smart_tput_col])).tolist()
        ...
        axs[csv_file_list.index(csv_file)].set_ylabel("Throughput (Mbps)",
                                                        fontsize=10)
    fig.savefig(f"figures/{operator}/single_{operator}_average_smart_tput")
